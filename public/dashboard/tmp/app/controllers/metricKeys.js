define([
  ...
  module.controller('MetricKeysCtrl', [
    '$scope',
    '$http',
    '$q',
    function ($scope, $http, $q) {
      ...
      if (config.elasticsearchBasicAuth) {
        ...
        httpOptions.headers = { 'Authorization': 'Basic ' + config.elasticsearchBasicAuth };
      }
      $scope.init = function () {
      ...
      $scope.createIndex = function () {
        ...
        $scope.infoText = null;
        deleteIndex().then(createIndex).then(function () {
      ...
      $scope.loadMetricsFromPath = function () {
        ...
        $scope.infoText = null;
        $scope.metricCounter = 0;
        return loadMetricsRecursive($scope.metricPath).then(function () {
      ...
      $scope.loadAll = function () {
        ...
        getFromEachGraphite('/metrics/index.json', saveMetricsArray).then(function () {
      ...
      function getFromEachGraphite(request, data_callback, error_callback) {
        return $q.all(_.map(config.datasources, function (datasource) {
          if (datasource.type = 'graphite') {
            return $http.get(datasource.url + request).then(data_callback, error_callback);
          }
        }));
      }
      function saveMetricsArray(data, currentIndex) {
        ...
        return saveMetricKey(data.data[currentIndex]).then(function () {
          return saveMetricsArray(data, currentIndex + 1);
        });
      }
      function deleteIndex() {
        var deferred = $q.defer();
        $http.delete(elasticSearchUrlForMetricIndex, httpOptions).success(function () {
          deferred.resolve('ok');
      ...
      function createIndex() {
        return $http.put(elasticSearchUrlForMetricIndex, {
          settings: {
            analysis: {
              analyzer: { metric_path_ngram: { tokenizer: 'my_ngram_tokenizer' } },
              tokenizer: {
                my_ngram_tokenizer: {
                  type: 'nGram',
                  min_gram: '3',
      ...
      function saveMetricKey(metricId) {
        ...
        var request = $scope.ejs.Document(config.grafana_metrics_index, 'metricKey', metricId).source({ metricPath: metricId });
