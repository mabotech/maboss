define([
  ...
  'use strict';
  angular.module('grafana.directives').directive('panelMenu', [
    '$compile',
    'linkSrv',
    function ($compile, linkSrv) {
      var linkTemplate = '<span class="panel-title drag-handle pointer">' + '<span class="panel-title-text drag-handle">{{panel.title | interpolateTemplateVars}}</span>' + '<span class="panel-links-icon"></span>' + '</span>';
      function createMenuTemplate($scope) {
        ...
        _.each($scope.panelMeta.menu, function (item) {
          ...
          if (item.editorLink) {
            template += ' dash-editor-link="' + item.editorLink + '"';
          }
          template += '>';
