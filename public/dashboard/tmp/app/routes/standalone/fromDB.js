define(['angular'], function (angular) {
  ...
  module.config([
    ...
    function ($routeProvider) {
      $routeProvider.when('/dashboard/db/:id', {
  ...
  module.controller('DashFromDBProvider', [
    '$scope',
    '$rootScope',
    'datasourceSrv',
    '$routeParams',
    function ($scope, $rootScope, datasourceSrv, $routeParams) {
      var db = datasourceSrv.getGrafanaDB();
      var isTemp = window.location.href.indexOf('dashboard/temp') !== -1;
      db.getDashboard($routeParams.id, isTemp).then(function (dashboard) {
  ...
  module.controller('DashFromImportCtrl', [
    '$scope',
    '$location',
    function ($scope, $location) {
      if (!window.grafanaImportDashboard) {
        $scope.appEvent('alert-warning', [
          'Dashboard load failed',
          'Cannot reload unsaved imported dashboard'
        ]);
        $location.path('');
        return;
      }
