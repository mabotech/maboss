define([
  ...
  module.factory('ElasticDatasource', [
    '$q',
    '$http',
    'templateSrv',
    function ($q, $http, templateSrv) {
      ...
      ElasticDatasource.prototype._get = function (url) {
        return this._request('GET', url, this.index).then(function (results) {
          return results.data;
        });
      };
      ElasticDatasource.prototype._post = function (url, data) {
        return this._request('POST', url, this.index, data).then(function (results) {
          return results.data;
        });
      };
      ...
      ElasticDatasource.prototype._getDashboardWithSlug = function (id) {
        return this._get('/dashboard/' + kbn.slugifyForUrl(id)).then(function (result) {
      ...
      ElasticDatasource.prototype.getDashboard = function (id, isTemp) {
        ...
        var self = this;
        return this._get(url).then(function (result) {
          return angular.fromJson(result._source.dashboard);
        }, function (data) {
          if (data.status === 0) {
      ...
      ElasticDatasource.prototype.saveDashboard = function (dashboard) {
        ...
        if (temp) {
          ...
          var self = this;
          return this._request('PUT', '/dashboard/' + id, this.index, data).then(function (results) {
      ...
      ElasticDatasource.prototype._removeUnslugifiedDashboard = function (saveResult, title, id) {
        if (saveResult.statusText !== 'Created') {
          return;
        }
        if (title === id) {
          return;
      ...
      ElasticDatasource.prototype._saveTempDashboard = function (data) {
        return this._request('POST', '/temp/?ttl=' + this.saveTempTTL, this.index, data).then(function (result) {
      ...
      ElasticDatasource.prototype.deleteDashboard = function (id) {
        return this._request('DELETE', '/dashboard/' + id, this.index).then(function (result) {
      ...
      ElasticDatasource.prototype.searchDashboards = function (queryString) {
        ...
        if (tagsOnly) {
          ...
          if (!queryString.match(/(\*|\]|}|~|\)|"|^\d+|\s[\-+]\w+)$/) && !queryString.match(/[A-Z]$/) && !endsInOpen(queryString, '(', ')') && !endsInOpen(queryString, '"', '"') && !endsInOpen(queryString, '[', ']') && !endsInOpen(queryString, '[', '}') && !endsInOpen(queryString, '{', ']') && !endsInOpen(queryString, '{', '}')) {
            queryString += '*';
          }
        }
        var query = {
        ...
        return this._post('/dashboard/_search', query).then(function (results) {
