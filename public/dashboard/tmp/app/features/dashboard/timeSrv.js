define([
  ...
  module.service('timeSrv', [
    '$rootScope',
    '$timeout',
    '$routeParams',
    'timer',
    function ($rootScope, $timeout, $routeParams, timer) {
      var self = this;
      ...
      this.timeRangeForUrl = function () {
        ...
        if (_.isDate(range.from)) {
          range.from = range.from.getTime();
        }
        if (_.isDate(range.to)) {
          range.to = range.to.getTime();
        }
        return range;
      };
