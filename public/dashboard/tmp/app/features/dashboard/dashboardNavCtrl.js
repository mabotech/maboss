define([
  ...
  module.controller('DashboardNavCtrl', [
    '$scope',
    '$rootScope',
    'alertSrv',
    '$location',
    ...
    'datasourceSrv',
    'timeSrv',
    function ($scope, $rootScope, alertSrv, $location, playlistSrv, datasourceSrv, timeSrv) {
      $scope.init = function () {
      ...
      $scope.saveForSharing = function () {
        ...
        clone.temp = true;
        $scope.db.saveDashboard(clone).then(function (result) {
          $scope.share = {
            url: result.url,
            title: result.title
          };
      ...
      $scope.passwordCache = function (pwd) {
        ...
        if (!pwd) {
          return window.sessionStorage['grafanaAdminPassword'];
        }
      ...
      $scope.isAdmin = function () {
        ...
        if ($scope.passwordCache() === config.admin.password) {
          return true;
        }
