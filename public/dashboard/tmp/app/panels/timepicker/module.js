  ...
  module.controller('timepicker', [
    ...
    function ($scope, $rootScope, timeSrv) {
      ...
      $scope.validate = function (time) {
        ...
        $scope.input.$setValidity('dummy', true);
        var _from = datepickerToLocal(time.from.date), _to = datepickerToLocal(time.to.date), _t = time;
    ...
    */
      ...
      var getScopeTimeObj = function (from, to) {
        ...
        if (timeSrv.time) {
          if ($scope.panel.now) {
            model.rangeString = moment(model.from.date).fromNow() + ' to ' + moment(model.to.date).fromNow();
          } else {
            model.rangeString = $scope.dashboard.formatDate(model.from.date, 'MMM D, YYYY HH:mm:ss') + ' to ' + $scope.dashboard.formatDate(model.to.date, 'MMM D, YYYY HH:mm:ss');
          }
        }
        return model;
      };
